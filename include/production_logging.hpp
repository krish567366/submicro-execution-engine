...
public:
    ...
    void log_rx_packet(uint64_t seq, const std::string& venue, uint64_t ts_hw_ns) {
        file_ << "RX_PKT seq=" << seq << " venue=" << venue << " ts_hw_ns=" << ts_hw_ns << "\n";
    }

    void log_tx_packet(uint64_t seq, const std::string& venue, uint64_t ts_hw_ns) {
        file_ << "TX_PKT seq=" << seq << " venue=" << venue << " ts_hw_ns=" << ts_hw_ns << "\n";
    }
...
public:
    ...
    void log_ack(uint64_t order_id, uint64_t exch_ts_ns) {
        file_ << "ACK order_id=" << order_id << " exch_ts_ns=" << exch_ts_ns << "\n";
    }

    void log_fill(uint64_t order_id, uint64_t qty, double price, uint64_t exch_ts_ns) {
        file_ << "FILL order_id=" << order_id << " qty=" << qty << " price=" << std::fixed << std::setprecision(4) << price << " exch_ts_ns=" << exch_ts_ns << "\n";
    }

    void log_reject(uint64_t order_id, const std::string& reason, uint64_t exch_ts_ns) {
        file_ << "REJECT order_id=" << order_id << " reason=" << reason << " exch_ts_ns=" << exch_ts_ns << "\n";
    }
...
public:
    ...
    void log_sync(uint64_t local_ts_ns, int64_t offset_ns, double drift_ppb) {
        file_ << "SYNC local_ts=" << local_ts_ns << " offset_ns=" << (offset_ns >= 0 ? "+" : "") << offset_ns 
              << " drift_ppb=" << std::fixed << std::setprecision(1) << (drift_ppb >= 0 ? "+" : "") << drift_ppb << "\n";
    }

    void log_gm_change(const std::string& old_gm, const std::string& new_gm, uint64_t ts_ns) {
        file_ << "GM_CHANGE old=" << old_gm << " new=" << new_gm << " ts=" << ts_ns << "\n";
    }
...
public:
    ...
    void log_submit(uint64_t order_id, const std::string& side, double price, uint64_t qty, uint64_t tsc) {
        file_ << "SUBMIT order_id=" << order_id << " side=" << side << " price=" << std::fixed << std::setprecision(4) << price << " qty=" << qty << " tsc=" << tsc << "\n";
    }

    void log_cancel(uint64_t order_id, uint64_t tsc) {
        file_ << "CANCEL order_id=" << order_id << " tsc=" << tsc << "\n";
    }
...
public:
    ...
    void finalize() {
        ...
        manifest.add_file("nic_rx_tx_hw_ts_" + run_id_ + ".log", "AUTO");
